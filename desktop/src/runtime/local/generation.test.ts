...
describe("localGenerationPort", () => {
  ...
  it("delegates retry, cancel, get and delete commands", async () => {
    ...
    invokeMock.mockResolvedValueOnce({ task: { id: "task_1" }, inputAssets: [], outputAssets: [], events: [] });
    ...
    await localGenerationPort.getTaskDetail("task_retry");
    ...
    expect(invokeMock).toHaveBeenNthCalledWith(5, "generation_delete_task", {
      taskId: "task_retry",
    });
  });
});
