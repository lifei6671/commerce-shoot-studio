...
const PRODUCT_DETAIL_ITEM_CONCURRENCY: usize = 1;
...
fn execute_product_detail_image_task(
    ...
    for batch in product_detail_input_batches(inputs.len(), PRODUCT_DETAIL_ITEM_CONCURRENCY) {
        for index in batch {
            ...
            let result =
                invoke_model_gateway(workspace_directory, capability_id, inputs[index].clone());
            match result {
                Ok(gateway_result) => {
                ...
                Err(source) => {
                    eprintln!(
                        "[local-task-executor] product detail item provider failed task_id={} item_index={} error={}",
                        task.id,
                        index,
                        source
                    );
                    failed_item_count += 1;
                    insert_task_item_failed_event(
                        workspace_directory,
                        &task.id,
                        index,
                        GenerationError::Validation(source.to_string()),
                    )?;
                }
            }
        }
    ...
    if saved_image_count == 0 || failed_item_count > 0 {
        let error = normalized_persist_error(task.kind);
        eprintln!(
...
fn product_detail_item_gateway_input(
    ...
    let user_prompt = format!(
        "请基于上传的商品参考图生成「{title}」。目标平台：{platform}。目标市场：{market}。目标语言：{language}。\n{locale_constraint}\n商品卖点：{product_selling_points}\n画面要求：{image_prompt}\n保持商品主体、颜色、版型和图案与参考图一致；画面中不得出现任何可读文字、汉字、英文、数字、标注条、说明牌、占位标签或文字框；需要放置文案的位置只保留干净空白构图，由后期叠字完成。"
    );
    let system_prompt = "你是专业电商商品详情页图生图生成器。必须严格参考用户上传商品图，保持商品事实一致，不编造品牌、参数、价格、销量或认证标识。";
...
fn sanitize_image_generation_prompt_text(value: &str) -> String {
    value
        .replace("预留文字安全区", "保留干净留白构图")
        .replace("文字安全区", "干净留白构图")
        .replace("只预留标题、副标题或参数信息区", "只保留干净留白构图")
        .replace(
            "不要求生图模型直接生成可读文字",
            "生图阶段不得绘制任何文字内容",
        )
        .replace("不直接生成可读文字", "不得绘制任何文字内容")
}
...
mod tests {
    use super::{
        ...
        mark_task_succeeded, product_detail_input_batches, product_detail_item_gateway_input,
        product_detail_output_sort_order_start, task_input_with_asset_reference_images,
        task_is_running, update_stage, ClaimedTask, BACKGROUND_TASK_CONCURRENCY,
    ...
    #[test]
    fn product_detail_item_batches_are_serial_inside_one_background_task() {
        assert_eq!(PRODUCT_DETAIL_ITEM_CONCURRENCY, 1);
        assert_eq!(
            product_detail_input_batches(4, PRODUCT_DETAIL_ITEM_CONCURRENCY),
            vec![0..1, 1..2, 2..3, 3..4]
        );
    }
