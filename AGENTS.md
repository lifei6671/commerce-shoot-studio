...
- provider 是否可用以持久化 `connectionStatus` 和连接指纹为准；provider、model、endpoint、API Key 任一变化后必须回到 `untested / 不可用`，直到测试成功。
...
- provider 可用性持久化在 `model_configs.connection_status` 等字段中；连接指纹包含 provider、模型、执行模式、endpointPath 和 secret version，修改 API Key、模型或接入路径后会自动回到 `untested`。
