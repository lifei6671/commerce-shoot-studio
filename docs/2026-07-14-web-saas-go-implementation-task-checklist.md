...
| [ ] G0-T06 建立 trusted proxy、请求 ID 与 CSRF 基线 | P0 | 待开始 | 待分配 | G0-T05、DS0-04 | 默认不信任代理；生成 request ID 并写响应 Header；用户/管理分别装配 `net/http.CrossOriginProtection` 拒绝处理器并复用 G0-T05 对应的 typed Origin 白名单，不维护第二套 trusted origin；不签发 CSRF Token/Cookie；响应 no-store/private、Vary Cookie | 待补充 |
