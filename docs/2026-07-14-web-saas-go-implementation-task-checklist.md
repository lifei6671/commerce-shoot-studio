...
| [x] DS0-05 确认 Secret 管理口径 | P0 | 已完成 | lifei6671（决策）/ Codex（记录） | DS0-04 | 冻结启动配置文件凭据与管理员后台 MySQL 密钥存储边界；不使用环境变量、Secret Manager 或独立密钥表 | 2026-07-15 用户确认 MySQL 密码、启用认证时的 Redis 密码等启动凭据写服务端配置文件；内网无认证 Redis 可显式使用空密码；`system_configs` 是业务固定 key + 完整 JSON value 的纯 KV 表，不使用 `value_type` 或凭据专用记录；S3/SMTP/OpenAI/火山等由管理员配置，普通用户不可配置，读取不回传密钥字段 |
...
| [x] DS0-09 确认用户端与管理端交互 | P0 | 已完成 | lifei6671（决策）/ Codex（记录） | DS0-03 | 批准 `/app` 认证/积分/上传路径、核心工作台 Web 适配、`/admin` 权限与危险操作流程、Ant Design/shadcn 边界 | 2026-07-15 已确认独立 Web/双组件、认证、响应式视觉、上传双入口、积分行内提示/逐图反馈和固定 `platform_admin`；管理信息架构选择 A“数据运营后台”：可折叠左侧导航，筛选栏 + AntD Table + 服务端分页，详情/简单编辑使用 Drawer，复杂配置独立页，危险操作 Modal，不增加展示型指标聚合接口；进一步确认本期不引入 MFA，不创建相关配置、API、UI 或发布门禁 |
...
| [x] G0-T02 建立配置加载与边界校验 | P0 | 已完成 | Codex（实现与核验） | G0-T01、DS0-05 | 单个 `app.yaml` 内联 MySQL/条件 Redis/用户与管理 Session/验证码启动凭据；以配置文件为起点，它的上一级目录（文件所在目录）为工作目录；`dsn_params` 支持后续自定义 DSN 参数；第一版单组 Base64 密钥，不做 key ring；建立 `system_configs` typed JSON 解码边界；不使用环境变量或 Secret Manager，无效配置快速失败；仓库只提交完整注释且无真实凭据的 `app.yaml.example`，实际 `app.yaml` 由 Git 忽略 | 2026-07-15 已完成 `server/conf/app.yaml.example`、`app.yaml` 精确忽略、`internal/config` 单文件严格加载/校验与 runtime typed JSON source Port；Viper key 归一化风险由 YAML mapping 预检及 camelCase DSN 原键恢复封闭。默认读取 `conf/app.yaml`，显式路径只读取指定文件且不回退模板；2026-07-15 追加确认开发目标为本地内网 MySQL 实例的 `commerce_shoot_studio` 库与无密码 Redis 实例，Redis 空密码合同及回归测试已补，连接地址与 MySQL 密码不进入仓库。`go1.26.5`、`gofmt -l`、`go mod tidy -diff`、`go mod verify`、`go vet ./...`、`go test ./...`、`go test -race ./...`、禁用 Viper 能力扫描、Git ignore 合同和 `git diff --check` 全部通过；真实 DB/Redis 连接按任务边界留给 G0-T07/G0-T08 |
| [x] G0-T03 建立整数错误与领域状态常量 | P0 | 已完成 | Codex（实现与核验） | G0-T01 | 错误号段、HTTP 映射、安全消息和按领域拆分的强类型 `uint8` 状态，不出现通用 `status.go` | 2026-07-15 用户确认最小冻结方案；已完成标准库 `lib/apperror`、六项首批 HTTP 错误、仅作任务结果码的 `140504`、Async Job `queued=0/running=1` 与四项 Model Category 强类型常量。TDD 先确认两个包无生产文件导致 RED，再实现转绿；`gofmt -l` 无输出，`go mod tidy -diff`、`go mod verify`、`go vet ./...`、`go test -count=1 ./...`、`go test -race -count=1 ./...`、`go run ./cmd/server`、依赖边界扫描和 `git diff --check` 全部通过；完整状态机与迁移规则留给 G0.5 |
| [x] G0-T04 建立 OpenAPI 合同与生成链 | P0 | 已完成 | lifei6671（方案 A 决策）/ Codex（实现与核验） | G0-T01、DS0-03 | 锁定 `oapi-codegen v2.7.2` / `openapi-typescript 7.13.0`；只读取仓库内受评审 schema，禁止远程或不受信任输入；首批冻结错误、分页/cursor、幂等 Header、AI 改写 SSE payload、报价和积分共享 components；初始 `paths: {}`，业务 operation 由所属任务在字段冻结后增量加入；不生成 CSRF Token 或静态 Cookie 名合同；生成 Go/TS DTO，生成物禁止手改且 diff 必审 | 2026-07-15 用户选择方案 A；已完成 `server/api/openapi.yaml`、models-only Go 生成配置、`internal/models/dto/generated/types.gen.go` 与 tooling-only 独立 `web/` npm 包、lockfile、`web/src/api/generated/openapi.ts`。TDD 先确认 schema/生成配置/产物缺失导致 RED，再实现转绿；合同测试覆盖 OpenAPI 3.0.3、内部 `$ref`、GET/POST only、operationId 唯一、CSRF/敏感字段拒绝、首批 component 精确字段白名单、三份 SHA-256 审查快照和 Desktop 依赖隔离。生成物进入 Git index 后双端二次生成 diff 为零；`gofmt -l`、`go mod tidy -diff`、`go mod verify`、`go vet ./...`、`go test -count=1 ./...`、`go test -race -count=1 ./...`、`go run ./cmd/server`、`npm ci`、`npm audit`（0 漏洞）、CycloneDX SBOM 和 `git diff --check` 通过；许可证清单已核对为 MIT/ISC/Apache-2.0/CC0/Python-2.0，策略自动化留 G0-T12；本机未安装 `govulncheck`，留 G0-T12 固化工具版本和 CI 门禁 |
| [x] G0-T05 建立 Gin 路由与 HTTP 边界 | P0 | 已完成 | Codex（实现与核验） | G0-T03、G0-T04、G0-T09 | `/api/v1`、`/api/admin/v1`、统一 403/404/405/413/500、默认 1 MiB body、32 KiB header、5s/30s/5m/60s server timeout、安全 recovery 与完成日志；用户/管理分别使用默认空的同站 CORS 白名单，合法预检 204、非法预检 403、普通 OPTIONS 405；HEAD 线路 body 为空，非标准方法日志归一化 `OTHER`；任何业务路由落地前完成边界测试 | 2026-07-15 已完成 Gin v1.12.0 与 x/net v0.51.0 精确依赖、HTTP/CORS 严格配置、100403/100404、generated `ErrorResponse` 唯一写出、双 API Group、同站双白名单 CORS、GET/POST-only 方法守卫、完整 JSON 绑定与 method+template body override、安全 recovery/完成日志、只构造不监听的 `http.Server` 及 AST build contract；复审追加关闭尾斜杠重定向、所有 API/OPTIONS Vary 缓存维度和尾随 JSON/超限字节拒绝。`gofmt -l`、`go mod tidy -diff`、`go mod verify`、`go vet ./...`、定向旧版 lint、定向/20 次 Shuffle/Race、`go test ./...`、`go test -race ./...`、`go build`、`go run` 与 `git diff --check` 全部通过；本机只有未锁定的 golangci-lint v1.64.8 且 `govulncheck` 未安装，正式 v2.12.2 lint/漏洞扫描留 G0-T12；用户 review 后已批准提交 |
| [x] G0-T06 建立 trusted proxy、请求 ID 与 CSRF 基线 | P0 | 已完成 | lifei6671（安全边界决策）/ Codex（实现与核验） | G0-T05、DS0-04 | 默认不信任代理；生成 request ID 并写响应 Header；用户/管理分别装配 `net/http.CrossOriginProtection` 并复用 G0-T05 对应的 typed Origin 白名单，不维护第二套 trusted origin；不签发 CSRF Token/Cookie；响应 no-store/private、Vary Cookie | 2026-07-15 完成 `security.trusted_proxy_cidrs` 严格配置、单跳 XFF/XFP/XFH typed resolver、forwarded Header 清理与 Gin shortcut 禁用；Router 启动期读取 32 字节进程密钥，请求期使用 HMAC-SHA256 + 原子序列生成 128-bit 小写十六进制 ID，统一绑定 Header/错误体/完成日志；用户/管理独立复用 canonical CORS map 执行 `CrossOriginProtection.Check`，API 全状态设置 no-store/private 与 Cookie/Origin/OPTIONS Vary。TDD 覆盖 IPv4/IPv6/mapped/zone、可信与非可信代理、严格 authority、200/403/404/405/413/500/HEAD/OPTIONS、并发 barrier、密钥/marker 泄漏、双 surface CSRF/cache 与 AST 绕过；三路安全复审 findings 修复后 no findings。`gofmt`、`go mod tidy -diff`、`go mod verify`、`go vet ./...`、`go test -count=1 -timeout=60s ./...`、`go test -race -shuffle=on -count=1 -timeout=60s ./...`、`go build ./cmd/server`、`go run ./cmd/server`、`git diff --check` 通过；未新增依赖、schema、OpenAPI operation 或真实配置文件改动，代码保持未提交供用户 review。 |
...
| [x] G0-T09 建立日志安全基线 | P0 | 已完成 | Codex（实现与核验） | G0-T01、DS0-02 | 业务只依赖 `slog`，使用获所有者授权的 logit v1.0.0 Handler；提供 transport-neutral fresh/fork context 与结束状态/错误码/耗时合同；完整配置和代表性恶意值的最终 writer 泄漏测试通过 | 2026-07-15 已完成 `server/lib/logger/{logger,safe_handler,context}.go`、黑盒/并发/递归 writer 测试及 `internal/buildcontract/logging_contract_test.go`；logit 精确锁定为唯一适配包的直接依赖，基础 JSON、typed context、状态分级、固定整数 attr 白名单、LogValuer 零调用、Config/JSON/error/URL/Base64 marker 禁泄漏和 go/types 源码绕过守卫均通过。`gofmt -l` 无输出，`go mod tidy -diff`、`go mod verify`、`go vet ./...`、定向测试、50 次 Shuffle、`go test ./...`、`go test -race ./...`、`go run ./cmd/server`、本任务包定向 lint 和 `git diff --check` 通过；本机 `golangci-lint` 为未锁定旧版 v1.64.8，全仓只命中既有 G0-T02 gosimple 建议，v2.12.2/CI 门禁留 G0-T12；`govulncheck` 与 SBOM 工具未安装，未临时引入，正式扫描和 logit 公开许可证据仍由 G0-T12/G7-T05 收口。真实 Gin/request ID/Tracing/Provider 出站接线未在本任务声称完成 |
...
- [ ] 非法 HTTP 方法、受控 OPTIONS 预检、用户/管理 CORS 隔离、CSRF、trusted proxy、health/readiness 合同测试通过。
...
| [ ] G2-T02 实现 Cookie Session Store | P0 | 待开始 | 待分配 | G2-T01、G0-T06、DS0-05 | fixation 防护、idle/absolute TTL、从部署配置文件分别加载用户/管理单组认证与加密密钥、替换密钥后对应 Session 全量失效、`session_version`、正确 Cookie Path；管理端不可在线修改 key | 待补充 |
...
| [ ] G2-T05 实现 Challenge/Grant Service | P0 | 待开始 | 待分配 | G0-T06、G2-T04、DS0-05 | 从部署配置文件加载单组独立 derivation key 与 verification key，替换后让未过期 challenge 全部失效；恒定时间比较、并发尝试上限和 grant 原子签发/消费 | 待补充 |
...
| [ ] G2-T10 验证 Redis Session Store | P1 | 待开始 | 待分配 | G2-T02、DS0-04、DS0-05 | Redis 启用认证时密码只从部署配置文件读取，同时覆盖无密码内网实例；选中 Redis 不可用时启动失败；验证单节点直连与可选 TLS；Sentinel/Cluster 不支持且不得出现在配置或能力声明中 | 待补充 |
...
| 任务 | 优先级 | 状态 | 负责人 | 依赖 | 可验收交付物 | 证据 |
| --- | --- | --- | --- | --- | --- | --- |
| [ ] G5-T01 在 G0-T04 tooling-only `web/` 上建立 `/app` 路由及双组件边界 | P0 | 待开始 | 待分配 | G0-T11、DS0-09 | 单 Web 产物、路由级拆包、同源 API；新增 SaaS 页面使用 Ant Design，核心工作台使用 shadcn/ui；隔离 Desktop/Web 展示层；宽屏三栏、平板抽屉、手机顶部+底部导航/单列分步；禁止页面复制领域状态机和 Remote Runtime 调用逻辑 | 待补充 |
...
| 任务 | 优先级 | 状态 | 负责人 | 依赖 | 可验收交付物 | 证据 |
| --- | --- | --- | --- | --- | --- | --- |
| [ ] G6-T01 确认 `/admin` 交互稿与组件边界 | P0 | 待开始 | 待分配 | DS0-09、DS0-10 | 独立 ConfigProvider、固定 `platform_admin` 鉴权、危险操作、近期认证、数据运营后台页面流程经用户确认；不建权限配置页或 MFA 交互 | 待补充 |
...
G6 退出条件：权限矩阵有自动化和人工证据；本期不引入 MFA，也不把 MFA 作为公网或发布门禁。
...
| 任务 | 优先级 | 状态 | 负责人 | 依赖 | 可验收交付物 | 证据 |
| --- | --- | --- | --- | --- | --- | --- |
| [ ] G7-T01 实现管理员高风险再认证与应急访问 | P0 | 待开始 | 待分配 | G6-T08 | 登录审计、充值/调价/对账的近期认证、恢复流程和应急访问策略；本期不实现 MFA | 待补充 |
...
- [ ] OpenAPI 业务路由只包含 GET/POST；PUT/PATCH/DELETE/HEAD/CONNECT/TRACE 与其他非标准方法返回 405 + 整数错误码，合法 OPTIONS 预检 204、非法预检 403、普通 OPTIONS 405，用户/管理 Origin 白名单互不串用。
